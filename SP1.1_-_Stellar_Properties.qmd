...
book:
  ...
  # execute:
  #   echo: false # This and the line above are to hide the codes
  author: Rosaria Lena
  date: 10/18/2023
  chapters:
    - index.qmd
...
- The parallax is generally measured in arcseconds (_as_). 1 arcsecond $= \dfrac{1}{3600} \deg = \dfrac{1}{206,265}$ radians. You can show these conversions by recalling that $1\ \rm{rad} = 57.3\deg = (57.3\times 60\times 60)\ \rm{as}=206,265\ \rm{as}$.

- The parsec (**par**allax-**sec**ond, _pc_) is defined as: $$ 1\ \rm{parsec} = \dfrac{1 au}{\rm{arcsecond\  in\  radians}} = 206,265\ au= \dfrac{648000}{\pi} \rm{au} \approx 3.086\times 10^{16}\rm{m}$$.
...
The __apparent brightness__, $F$  (units [$\rm{W/m}^2$]), also referred to as the _radiant flux_ or _power flux_, is the total amount of energy emitted by a star, per unit time, crossing an unit area. It is _not_ an intrinsic property of a star, because it depends on the distance from the star.
