...
![Extension of the log-log graph of pressure vs density (@fig-log-pressure), showing an additional line for the _relativistic_ electron degeneracy pressure.](img/logrhologP2.jpg)
...
- Remember that from the  Heisenberg Uncertainty Principle all the confined particles have the same momentum, $p\approx\hbar/\Delta x$.
- Since $m_{\mathrm{p}}/m_{\mathrm{e}}\approx 1800$ and $v = p/m$, the velocity of the protons will be about a factor of _1800 times less_ than that of the electrons.
- So the protons are _still non-relativistic_.  Their degeneracy pressure is still given by @eq-partial-pressure:

$$
P_{\mathrm{Qp}}=\frac{\beta}{2}\frac{\hbar^{2}}{m_{\mathrm{p}}}\left(\frac{\rho}{m_{\mathrm{p}}}\right)^{5/3}.
...
$$ 
p^{+} + e^{-} \rightarrow n + v_{\mathrm{e}},
$$

merging to form neutrons. Thus, these stellar remnants are called __neutron stars__.

![Extension of the log-log graph of pressure vs density (@fig-log-pressure-relativistic), showing an additional line for the (non-relativistic) proton degeneracy pressure.](img/logrhologP3.jpg){#fig-log-pressure-protons}
...
In @eq-quantum-pressures have shown that our equations for $P_{\mathrm{Qe}}$ and $P_{\mathrm{Qp}}$ are identical with the exception of particle mass
$$
  P_{\mathrm{Qe}}=
  \frac{\beta\hbar^{2}}{2m_{\mathrm{e}}}\left(\frac{\rho}{m_{\mathrm{p}}}\right)^{5/3},
$${#eq-quantum-pressure-el}
$$
P_{\mathrm{Qp}} = \frac{\beta\hbar^{2}}{2m_{\mathrm{p}}}\left(\frac{\rho}{m_{\mathrm{p}}}\right)^{5/3}.
$${#eq-quantum-pressure-p}
...
In Chapter 9, equating the electron degeneracy pressure @eq-quantum-pressure-el and the central pressure @eq-central-pressure, we found that the density of white dwarfs (@eq-wd-density) is
$$
\rho_{\mathrm{WD}}=\frac{4\pi}{3\beta^{3}}G^{3}\frac{m_{\mathrm{e}}^{3}m_{\mathrm{p}}^{5}}{\hbar^{6}}M^{2}.
$$
   
We can proceed analogously to find the density of neutron stars, using @eq-quantum-pressure-p to equate the proton degeneracy pressure and the central pressure @eq-central-pressure. We obtain
$$
\rho_{\mathrm{NS}}=\left(\frac{m_{\mathrm{p}}}{m_{\mathrm{e}}}\right)^{3}\rho_{\mathrm{WD}}=\frac{4\pi}{3\beta^{3}}G^{3}\frac{m_{p}^{8}}{\hbar^{6}}M^{2}.
$${#eq-ns-density}
...
$$
R_{\mathrm{WD}}=\left(\frac{3}{4\pi}\right)^{2/3}\frac{\beta}{G}\frac{\hbar^{2}}{m_{\mathrm{e}}m_{\mathrm{p}}^{5/3}}M^{-1/3},
...
$$
\begin{aligned} 
  R_{\mathrm{NS}}&=\frac{m_{\mathrm{e}}}{m_{\mathrm{p}}}R_{\mathrm{WD}}\nonumber\\
 &=\left(\frac{3}{4\pi}\right)^{2/3}\beta\frac{\hbar^{2}}{Gm_{\mathrm{p}}^{8/3}}M^{-1/3}\nonumber\\
 \nonumber \\
  &\approx 1.2\times
10^{3}\left(\frac{M}{M_{\odot}}\right)^{-1/3}\,\mathrm{m}\nonumber
\end{aligned}
$${#eq-ns-radius}
...
extremely hot. $T > 10^{6}\,\mathrm{K}$ - glows white (and other important emission mechanisms as well: see later lectures).
- Extremely small: $\sim 10^{4}\,\mathrm{m}$.
- Absurdly high density: $\sim 10^{17}$ to $10^{18}\,\mathrm{kg m}^{-3}$ (i.e. $1\,\mathrm{cm}^{3}$ of neutron star material weighs about a gigaton).

## Limit of neutron star mass: the Tolman-Oppenheimer-Volkoff Mass Limit

- Note that, just as in white dwarfs, the radius of a neutron star varies inversely with the cube root of its mass: $R_{\mathrm{NS}}\propto M^{-1/3}$
...
- The critical mass above which neutron degeneracy is unable to support the neutron star in a stable equilibrium is called the Tolman-Oppenheimer-Volkoff Mass Limit, $M_{\mathrm{TOV}}$.

- Calculation of the value of $M_{\mathrm{TOV}}$ is a difficult problem, which requires General Relativity to describe the gravitational effects and an accurate Equation of State for the neutron-degenerate matter. The latter is still the subject of continuing astrophysical research. Consequently, $M_{\mathrm{TOV}}$
is still fairly imprecisely known, probably lying somewhere between 1.5 and 3.0 $M_{\odot}$.
- Measurement of the mass of the neutron star in PSR J0348+0432 $(2.01\pm 0.04)$M$_{\odot}$ puts a lower bound on $M_{\mathrm{TOV}}$.
- Thus we can take $M_{\mathrm{TOV}} \approx 3.0M_{\odot}$ as the maximum neutron star mass.
...
![Extension of @fig-log-pressure-protons to the relativistic proton degeneracy limit.](img/logrhologP4.jpg){#fig-log-pressure-rel-protons}
...
- Initially, the $P_{\mathrm{c}}$ and $P_{\mathrm{QpR}}$ curves run parallel, so do not intersect.  As the density increases, $P_{\mathrm{c}}$ actually starts to increase faster than $\rho^{4/3}$, so the $P_{\mathrm{c}}$ curve diverges away from the $P_{\mathrm{QpR}}$ curve.
...
$$
  g(r) = \frac{GM}{r^{2}}\frac{1}{\sqrt{1-R_{\mathrm{S}}/r}},
$${#eq-gravity-accel-rel}
where $R_{\mathrm{S}}$ is the Schwarzschild Radius.

The __Schwarzschild Radius__ is given by
$$
R_{\mathrm{S}}=\frac{2GM}{c^{2}}.
$${#eq-schwarzschild-radius}
So, as $r\rightarrow R_{\mathrm{S}}$, then $g(r)\rightarrow \infty$, and so $P_{\mathrm{c}}\rightarrow \infty$.
...
- __2__ As the particles become relativistic, we can effectively think of their masses increasing (Tolman). The particles still have the same _rest mass_ but their total _energy and momentum_ increase without bound.
  ...
  $$
  m_{\mathrm rel} =\gamma m_{0}=\frac{m_{0}}{\sqrt{1-v^{2}/c^{2}}}\nonumber
  $${#eq-relat-particle-mass}
   - As the density of the remnant increases, the speeds of its constituent particles increase, so the relative particle masses increase. This causes a rise in the central pressure due to self-gravity, $P_{\mathrm{c}}$.

With $P_{\mathrm{c}}$ climbing unboundedly, no known physical process is able to halt the contraction. 
...
- From the perspective of an external observer, the mass disappears from view when its radius falls below $R_{\mathrm{S}}$.
- $R_{\mathrm{S}}$ is the radius of the event horizon. The object has become a __black hole__.

## Observational Properties of Neutron Stars

__How do we observe Neutron Stars?__
...
$$
L=\frac{2}{5}MR_{\mathrm{NS}}^{2}\frac{2\pi}{\tau_{\mathrm{NS}}} 
$${#eq-ang-mom-ns}

So the rotation period of the neutron star will be given by
$$
\begin{aligned}
\frac{R_{0}^{2}}{\tau_{0}}&=\frac{R_{\mathrm{NS}}^{2}}{\tau_{\mathrm{NS}}}\\
\tau_{\mathrm{NS}}&=\left(\frac{R_{\mathrm{NS}}}{R_{0}}\right)^{2}\tau_{0}\nonumber
\end{aligned}
$${#eq-ns-period}
...
rotation period of $t_{\mathrm{NS}} \approx 0.0003$ s.
...
$$
  \begin{aligned}
  m\omega^{2}_{\mathrm{max}}R&=\frac{GMm}{R^{2}}\nonumber\\
  \omega_{\mathrm{max}}&=\sqrt{\frac{GM}{R^{3}}}.\nonumber
  \end{aligned}
$${#eq-ns-max-rot-speed}
...
$$
\tau_{\mathrm{min}}=\frac{2\pi}{\omega_{\mathrm{max}}}=2\pi\sqrt{\frac{R^{3}}{GM}}.\nonumber
...
$${#eq-rot-kin-en}
  ...
  - This is the rotational equivalent of the kinetic energy associated with linear motion, given by the familiar expression $(1/2)mv^{2}$: for rotations the mass is replaced by the moment of inertia and the linear speed is replaced by the angular speed
  - Hence, using the moment of inertia of @eq-mom-inertia-sphere and the maximum angular speed of @eq-ns-max-rot-speed, the maximum rotational energy content of a neutron star will be given by
$$
E_{\mathrm{max}}=\frac{1}{2}\frac{2}{5}M_{\mathrm{TOV}}R^{2}\omega_{\mathrm{max}}^{2}=\frac{1}{5}\frac{GM_{\mathrm{TOV}}^{2}}{R}.
$${#eq-max-rot-en}

### <span style="color:#336699;">Question</span>

What is the minimum rotation period of a neutron star at the __Tolman-Oppenheimer-Volkoff Mass Limit__ $M_{\mathrm{TOV}} \approx 3M_{\odot}$, having a radius of $\sim 10$ km?
...
Using @eq-ns-min-rot-period (remember to convert the radius from km to m), we find that the neutron star will have a minimum rotation period of $t_{\mathrm{min}} \approx 0.3$ milliseconds.
...
- Evaluating the maximum value, we find $E_{\mathrm{max}} \approx 10^{46}$ J.
- For a typical neutron star (such as the Crab), $E_{\mathrm{rot}} \approx 10^{41}$ J.
- The most energetic nuclear weapon was $\sim 10^{17}$ J.

### Magnetic Fields in Neutron Stars

- Neutron stars have _extremely strong magnetic fields_, between $10^4$ and $10^{11}$ T!
- For reference: the Earth's magnetic field is about $10^{-5}$ T, and the strongest lab magnets has a magnetic field strenght of about $10$ T.
- However, before collapsing stars do not have such strong magnetic fields!

_Why do neutron stars have such high magnetic field strenghts?_
...
![Dipole magnetic field in a collapsing star with initial radius and mass respectively $R_0$ and $M_0$.](img/bfield2.jpg){#fig-bfield-dipole}
...
R_{\mathrm{NS}}^{2}B_{\mathrm{NS}}\nonumber\\
B_{\mathrm{NS}}&=\left(\frac{R_{0}}{R_{\mathrm{NS}}}\right)^{2}B_{0}.\nonumber
\end{aligned}
$${#eq-ns-bfield}

### <span style="color:#336699;">Question</span>

Consider a dying star of radius $10^{6}$ km having a global magnetic field of $0.001$ T. What would the magnetic field be, after collapsing to a neutron star of radius $10$ km?
...
```
 ...
 __This is incredibly strong__! As mentioned above, compare to the Earth's magnetic field of about $10^{-5}$ T, or the strongest lab magnets of about $10$ T.
...
- The axis of rotation does not usually coincide with the magnetic axis (see @fig-neutron-star-magnetic-field).
...
![Magnetic field lines in a neutron star. The magnetic axis does not overlap with the rotation axis.](img/neutron-stars-magnetic-field.jpeg){#fig-neutron-star-magnetic-field}

_What happens when there is a time-varying magnetic field?_
...
- The plasma physics of this situation is highly complicated and not completely understood. However, the general outcome is the production of strong _beams_ of electrons, which stream out from the neutron star's _magnetic poles_, where the field is strongest (see @fig-neutron-star-magnetic-field, @fig-el-beaming-scheme and @fig-m87-beaming).
...
_Are these pulses regular or irregular?_
...
- Irregularities can be associated due to gravitational waves and [can be used for gravitational waves detections](https://physicsworld.com/a/pulsar-timing-irregularities-reveals-hidden-gravitational-wave-background/).

### Changes in pulsar period

The pulses of radiation from a pulsar are extremely regular in the sense that they are highly predictable. However, they do not
arrive at a perfectly constant rate. The pulse rate is observed to decline slowly.
...
:::::: {.callout-caution}
Careful not to confuse the rotation period (here called $P$) with the pressure!
:::

- Then we can define the characteristic __spin-down time__ $t_{\mathrm{s}}$ as the length of time it would take for the pulsar's rotation to cease, if its current rate of decline continued unchanged:
$$
t_{\mathrm{s}}=\frac{P}{\dot{P}}\nonumber
$${#eq-spindown-time}

- For observed pulsars, $t_{\mathrm{s}}$ is measured to be $\sim 10^{4}$ to $10^{7}$ years.
...
- Most pulsars have $0.1\mathrm{s}<P<1\mathrm{s},$ and $\dot{P}\sim 10^{-15}$s/s as they have been spinning down for a long time.
...
- Short period (millisecond) pulsars would then also have to be very young, and should still be surrounded by their supernova remnants, e.g. the Crab pulsar, with rotation period $P = 33$ ms, embedded in the Crab Nebula supernova remnant (see previous chapter).
...
$$
\begin{aligned}
M_A&=1.34M_{\odot},\,P_A=23\,\mathrm{ms}\\
M_B&=1.25M_{\odot},\,P_B=2.8\,\mathrm{s}
\end{aligned}
$$
