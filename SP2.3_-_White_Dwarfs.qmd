...
$$
  P_{\mathrm{c}}=\left(\frac{\pi}{6}\right)^{1/3}GM^{2/3}\rho^{4/3},
...
$$
{P_{\mathrm{Qe}}=\frac{\beta}{2} \frac{\hbar^{2}}{m_{\mathrm{e}}}\left(\frac{\rho}{m_{\mathrm{p}}}\right)^{5/3}}
...
Equating $P_{\mathrm{Qe}}$ and $P_{\mathrm{c}}$, we have (solving for the density of the white dwarf $\rho=\rho_{WD}$)
$$
\begin{aligned}
  \frac{\beta}{2}\frac{\hbar^{2}}{m_{\mathrm{e}}}\left(\frac{\rho}{m_{\mathrm{p}}}\right)^{5/3}&=\left(\frac{\pi}{6}\right)^{1/3}GM^{2/3}\rho^{4/3}\\
      \rho^{1/3}&=\left(\frac{\pi}{6}\right)^{1/3}GM^{2/3}\frac{2}{\beta}\frac{m_{\mathrm{e}}}{\hbar^{2}}m_{\mathrm{p}}^{5/3}\\
\end{aligned}
$$
$$
...
\begin{aligned}
      \frac{M}{\frac{4}{3}\pi
        R^{3}}&=\frac{4\pi}{3\beta^{3}}G^{3}\frac{m_{\mathrm{e}}^{3}m_{\mathrm{p}}^{5}}{\hbar^{6}}M^{2}\nonumber\\
      \frac{1}{R^{3}}&=\left(\frac{4\pi}{3}\right)^{2}\left(\frac{G}{\beta}\right)^{3}\frac{m_{\mathrm{e}}^{3}m_{\mathrm{p}}^{5}}{\hbar^{6}}M\nonumber\\
      R&=\left(\left(\frac{4\pi}{3}\right)^{2}\left(\frac{G}{\beta}\right)^{3}\frac{m_{\mathrm{e}}^{3}m_{\mathrm{p}}^{5}}{\hbar^{6}}M\right)^{-1/3}\nonumber\\ 
\end{aligned}
$$
$$
...
$$
\begin{aligned}
     R_{\mathrm{WD}}&=2.986\times 10^{16}M^{-1/3}\,\mathrm{m}\nonumber\\
     R_{\mathrm{WD}}&=2.986\times
     10^{16}\left(\frac{M}{M_{\odot}}\right)^{-1/3}M_{\odot}^{-1/3}\,\mathrm{m}\\
     R_{\mathrm{WD}}&\approx 2\times
10^{6}\left(\frac{M}{M_{\odot}}\right)^{-1/3}\,\mathrm{m}
\end{aligned}
$${#eq-wd-radius-meters}
...
We have shown that the radius of a white dwarf (@eq-wd-radius and @eq-wd-radius-meters) is $R_{\mathrm{WD}}\propto M^{-1/3}$.
...
$$
p_{x}\approx\Delta p_x\approx \frac{\hbar}{\Delta x}\approx\hbar \left(\frac{\rho}{m_{\mathrm{p}}}\right)^{1/3}.
$${#eq-momentum-hup}

So at the Chandrasekhar mass limit (the mass at which relativistic effects become relevant)
$$ 
p\approx\hbar \left(\frac{\rho}{m_{\mathrm{p}}}\right)^{1/3}\approx m_{\mathrm{e}}c.
$${#eq-momentum-relativistic}

From our expression for the White Dwarf density (@eq-wd-density) we can now write
$$
\hbar\left(\frac{1}{m_{\mathrm{p}}}\frac{4\pi}{3\beta^{3}}G^{3}\frac{m_{\mathrm{e}}^{3}m_{\mathrm{p}}^{5}}{\hbar^{6}}M^{2}\right)^{1/3}\approx
m_{\mathrm{e}}c.
...
$$
\begin{aligned}
      \hbar^{3}\frac{1}{m_{\mathrm{p}}}\frac{4\pi}{3\beta^{3}}G^{3}\frac{m_{\mathrm{e}}^{3}m_{\mathrm{p}}^{5}}{\hbar^{6}}M^{2}&\approx
    m_{\mathrm{e}}^{3}c^{3}\nonumber\\
    \frac{4\pi}{3\beta^{3}}G^{3}\frac{m_{\mathrm{p}}^{4}}{\hbar^{3}}M^{2}&\approx
    c^{3}\nonumber
\end{aligned}
$${#eq-chandrasekar-mass-derivation}
$$
M_{\mathrm{Ch}}\approx\left(\frac{3}{4\pi}\right)^{1/2}\frac{1}{m_{\mathrm{p}}^{2}}\left(\frac{\beta\hbar c}{G}\right)^{3/2}
$${#eq-chandrasekar-mass}

- $M_{\mathrm{Ch}}$ is called the Chandrasekhar Mass Limit, and is the __maximum mass__ that a white dwarf can be.
...
$$
P_{\mathrm{QeR}}\propto nE_{\mathrm{eR}}\propto\frac{\rho}{m_{\mathrm{p}}}E_{\mathrm{eR}}\propto\frac{\rho}{m_{\mathrm{p}}}pc\propto\rho^{4/3}.
...
Above the Chandrasekhar mass, the curves for the central pressure due to self-gravity, $P_{\mathrm{c}}$, and the relativistic electron degeneracy pressure, $P_{\mathrm{QeR}}$, are __parallel__. Therefore,
these curves will no longer intersect.

So above $M_{\mathrm{Ch}}$, there is no value of $R$ for which $P_{\mathrm{QeR}}$ balances $P_{\mathrm{c}}$. No equilibrium is possible, and so the stellar
...
$$
\lambda_{\mathrm{max}}T=b\hspace{1cm}b=2.898\times 10^{- 3}\,\mathrm{m K}.
...
\begin{aligned}
        M&=11.18\nonumber\\
        L&=0.023L_{\odot}\nonumber\\
        R&=0.008R_{\odot}=5.7\times 10^{6}\,\mathrm{m}\nonumber\\
        \rho&=2.5\times 10^{9}\,\mathrm{kg}\,\mathrm{m}^{-3}\nonumber
\end{aligned}
$$
...
\begin{aligned}
        ...
        M&=0.5M_{\odot},\hspace{0.2cm}T=16,500\,\mathrm{K}\nonumber
\end{aligned}
$${#eq-erib-properties}
...
$$
  L_{\mathrm{acc}}=\frac{dE}{dt}=\frac{GM}{R}\frac{dm}{dt}=\frac{GM}{R}\dot{m},
$${#eq-accretion-luminosity}
where $\dot{m}$ is the _mass accretion rate_ (kg s$^{-1}$).

- If we assume the accretion disc radiates like a __blackbody__, we can relate the disc accretion luminosity to the disc temperature $T_{\mathrm{d}}$ using the Stefan-Boltzmann law (@eq-total-luminosity):
$$
L_{\mathrm{acc}}=A\sigma T^{4}_{\mathrm{d}}.
$${#eq-accretion-luminosity2}

- Note that in the formula above, since the emitting area $A$ is both sides of a flat disc, rather than the usual sphere, the emitting area for an accretion disc of radius $R_{\mathrm{d}}$ is
$$
      A=2\pi R^{2}_{\mathrm{d}}.
$${#eq-flat-disc-area}

- Equating our expressions for luminosity, @eq-accretion-luminosity and @eq-accretion-luminosity2, we find
$$
L_{\mathrm{acc}}=\frac{GM\dot{m}}{R}=2\pi
R_{\mathrm{d}}^{2}\sigma T^{4}_{\mathrm{d}}.
$${#eq-accretion-luminosity-12}

__Accretion disks__ are usually observed in X-rays, giving $T_{\mathrm{d}}\approx 10^{6}$ K.  Orbital measurements of the binary system can give $M$, and thus $R$ from the mass-radius relationship. If the disc can be resolved, $R_{\mathrm{d}}$ can be determined, thus allowing us to calculate $\dot{m}$
from measurements of $L_{\mathrm{acc}}$.
...
This fusion releases a large amount of energy, causing a sudden bright outburst of emission - a __nova__. This thermonuclear explosion temporarily sweeps away the accretion disk: when it builds up again sufficiently, another nova occurs.
...
- If the mass of an accreting carbon white dwarf approaches $M_{\mathrm{Ch}}$, the density within the star becomes sufficiently high that rapid carbon fusion can begin throughout the entire star - a _carbon detonation supernova_ (Type 1a). 
