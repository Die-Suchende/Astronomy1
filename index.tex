...
\item
  ...
  as:
  \[ 1\ \mathrm{parsec} = \dfrac{1 au}{\mathrm{arcsecond\  in\  radians}} = 206,265\ au= \dfrac{648000}{\pi} \mathrm{au} \approx 3.086\times 10^{16}\mathrm{m}\].
\item
...
\(\alpha\approx 0.5^\circ \approx 32 \ \mathrm{arcmin} \approx 9.3\times 10^{-3} \mathrm{rad}\),
and we know its distance (from Earth)
\(D = 1 \mathrm{au} = 149,597,870 \ \mathrm{km}\), we can now calculate
the size of the Sun.
...
The \textbf{apparent brightness}, \(F\) (units
{[}\(\mathrm{W/m}^2\){]}), also referred to as the \emph{radiant flux}
or \emph{power flux}, is the total amount of energy emitted by a star,
per unit time, crossing an unit area. It is \emph{not} an intrinsic
property of a star, because it depends on the distance from the star.
...
Sun's luminosity \(L_\odot = 3.826 \times 10^{26}\mathrm{W}\).
...
and \(L_2\) at the same distance \(r=10\mathrm{pc}\), their difference
in the apparent magnitudes is the same as the difference of the absolute
magnitudes:
